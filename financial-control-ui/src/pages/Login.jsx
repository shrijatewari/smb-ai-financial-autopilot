...
export default function Login() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-white via-violet-50/90 to-violet-100/80 px-4">
      ...
      <div className="pointer-events-none absolute -right-24 bottom-10 h-64 w-64 rounded-full bg-emerald-400/15 blur-[90px]" />
      <motion.div
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          ...
          <label className="block">
            <span className="text-xs font-medium text-violet-800/80">Password</span>
            <input
              type="password"
              required
              value={password}
        ...
        <p className="mt-6 text-center text-sm text-violet-950/55">
          No account?{' '}
          <Link to="/signup" className="font-medium text-[#6C3BFF] underline-offset-2 hover:underline">
            Create one
          </Link>
        </p>
      </motion.div>
